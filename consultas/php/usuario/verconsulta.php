...
$query_estado = "SELECT * FROM consultas WHERE radicado = '$id'";
$result_estado = $db_connection->query($query_estado);
...
<div style="background-color: orange; width:100%; height:20px; "></div>
  ...
  <div class="flex items-center justify-between w-full md:flex md:w-auto md:order-1  mx-auto" id="navbar-user">
    ...
    <li>
        <a href="consultorio.php" class="flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900  hover:text-orange-500">Home</a>
      </li>
   
     
      <li>
        <a href="perfil.php" class="flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900">Perfil</a>
      </li>
      <li>
        <a href="acercade.php" class="flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900">Acerca de</a>
      </li>
    </ul>
  </div>
  </div>
...
if(isset($_GET['success']) && $_GET['success'] == 1) {
  echo "<div id='mensajeActualizar'>¡Su consulta se ha actualizado correctamente!</div>";
}
?>
...
if(isset($_GET['errorActualizar']) && $_GET['errorActualizar'] == 1) {
  echo "<div id='errorActualizar'>¡Su consulta se ha actualizado correctamente!</div>";
}
?>
...
<section class="informacion">
  ...
  <div class="mt-6 border-t border-gray-100">
    <dl class="divide-y divide-gray-100">
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm font-medium leading-6 text-gray-900">RADICADO</dt>
        <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"><?php echo($radicado) ?></dd>
      </div>
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm font-medium leading-6 text-gray-900">FECHA</dt>
        <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"><?php echo($fecha) ?></dd>
      </div>
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
      ...
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm font-medium leading-6 text-gray-900">PERFIL</dt>
        <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"><?php echo($profesion) ?></dd>
      </div>
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
      ...
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm font-medium leading-6 text-gray-900">CONSULTA</dt>
        <dd class="mt-1 text-sm leading-6 <?php echo ($estado == 'RESPONDIDA') ? 'text-green-500' : 'text-red-500'; ?> sm:col-span-2 sm:mt-0"><?php echo($estado) ?></dd>
 ...
 <section style="display: flex;   flex-direction: row;  margin-top: 40px;">

 <?php if ($estado != 'RESPONDIDA'): ?>
    <button style="width: 25%; background-color: orange; margin: 0 auto" data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" class="block text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
        EDITAR
    </button>
<?php endif; ?>
 
 <?php if ($estado != 'RESPONDIDA'): ?>
    <button style="width: 25%; background-color: red; margin: 0 auto"  data-modal-target="popup-modal2" data-modal-toggle="popup-modal2" class="block text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
        ELIMINAR
    </button>
<?php endif; ?>
</section>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Estas seguro de borrar su consulta?</h3>
                <button onclick="confirmAction('<?php echo $id; ?>')"  class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                    si, seguro
                </button>
...
<script>
      function confirmAction(id) {
        // Redirigir a otra página con la variable $id en la URL
        window.location.href = 'eliminarconsulta.php?id=' + id;
    }
</script>
</article>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5">
                <form class="space-y-4" action="actualizarConsulta.php" method="POST">
                    <div>
                  ...
                  <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Consulta</label>
